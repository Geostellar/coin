...
contract('ERC20Allowances Contract', accounts => {
    ...
    it("should allow a third party-autorization", async () => {
    ...
    it("should allow authorized third-party transfers to take place", async () => {
        ...
        assert.equal(balance.valueOf(), 10, "Account did not receive the tird party transfer");
    });

    it("should allow not allow transfering more tokens than allowed", async () => {
        await campaign.approve(accounts[1], 10, {from: accounts[0]})
